(function() {
    ...
    $(window).on('load resize', function() {
        ...
        if ($(window).width() > 767) {
            $('.rsm-block-img').height($('.rsm-block-content').outerHeight(true));
        } else {
            $('.rsm-block-img').height(300);
        }
        ...
        $('.wpc-newsletter-img').height($('.wpc-newsletter-info').outerHeight());

        // agency, business, 
        if (winW >= 992) {
            $('.wpc-2block-img').height($('.wpc-2block-info').height());
            $('.wpc-business-img').height($('.wpc-buisness-info').height());
            $('.wpc-2block-img2').height($('.wpc-2block-info2').height() + 156);
        } else if (winW >= 768) {
            $('.wpc-2block-img').height('500');
            $('.wpc-business-img').height('500');
            $('.wpc-2block-img2').height('500');
        } else {
            $('.wpc-2block-img').height('300');
            $('.wpc-business-img').height('300');
            $('.wpc-2block-img2').height('300');
        }
        ...
        if ($('.izotop-full-height').length && $(window).width() >= 768) {
            $('.izotop-full-height').height($(window).height() - 80).css('marginTop', '80px')
            $('.izotop-full-height.gutt-col3 .item').css({
                "marginTop": "0px",
                "marginBottom": "0px"
            })
        } else {
            $('.izotop-full-height').css('marginTop', '80px');
            $('.izotop-full-height.gutt-col3 .item').css({
                "marginTop": "1%",
                "marginBottom": "1%"
            })
    ...
    function blockInfoItem() {
        if ($('.wpc-2block-info-item').length) {
            if (winW > 600) {
                block_height('.wpc-2block-info-item');
            } else {
                $('.wpc-2block-info-item').height('auto');
            }

        }
    }
    function block2InfoItem() {
        if ($('.wpc-2block-info2-item').length) {
            if (winW > 600) {
                block_height('.wpc-2block-info2-item');
            } else {
                $('.wpc-2block-info2-item').height('auto');
            }

        }
    }
    function buisnessInfoItem() {
        if ($('.wpc-buisness-info-item').length) {
            if (winW > 600) {
                block_height('.wpc-buisness-info-item');
            } else {
                $('.wpc-buisness-info-item').height('auto');
            }
        }
    }
    function helpInfoItem() {
        if ($('.wpc-help__item').length) {
            if (winW > 991) {
                block_height('.wpc-help__item');
            } else {
                $('.wpc-help__item').height('auto');
            }
        ...
        $(window).on('scroll', function() {
            ...
            if ($(window).width() > 768 && $('.parallax-efect').length ) {
                ...
                if ($(window).scrollTop() >= 20) {
                    $(".parallax1").css("margin-top", ($(window).scrollTop() / 5))
                    $(".parallax2").css("margin-bottom", ($(window).scrollTop() / 2))
                    console.log('test');
                } else {
                    $(".parallax1").css("margin-top", (0))
                    $(".parallax2").css("margin-bottom", (0))
    ...
    function startCharts() {
        if($('.pie_progress').length) {
            for (var i = 0; i < $('.pie_progress').length; i++) {
                if ($(window).scrollTop() >= $($('.pie_progress')[i]).offset().top - $(window).height() * 0.9) {
                    $($('.pie_progress')[i]).asPieProgress('start');
                }
            }
        }
    }
